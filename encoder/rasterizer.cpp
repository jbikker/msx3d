...
Mesh::Mesh( int vcount, int tcount ) : verts( vcount ), tris( tcount )
{
	...
	spos = new vec2[vcount * 2], N = new vec3[tcount];
...
void Mesh::Render( mat4& transform )
{
	...
	for( int i = 0; i < tris; i++ )
	{
		...
		// clip
		vec3 cpos[2][8], *pos;
		int nin = 3, nout = 0, from = 0, to = 1, miny = 212 - 1, maxy = 0, h;
		for( int v = 0; v < 3; v++ ) cpos[0][v] = tpos[tri[i * 3 + v]];
		for( int p = 0; p < 2; p++, from = 1 - from, to = 1 - to, nin = nout, nout = 0 ) for( int v = 0; v < nin; v++ )
		{
			...
			if ((t1 < 0) && (t2 >= 0))
				f = t1 / (t1 - t2),
				cpos[to][nout++] = A + f * (B - A),
				cpos[to][nout++] = B;
			else if ((t1 >= 0) && (t2 >= 0)) cpos[to][nout++] = B;
			else if ((t1 >= 0) && (t2 < 0))
				f = t1 / (t1 - t2),
				cpos[to][nout++] = A + f * (B - A);
		}
		...
		pos = cpos[from];
		for( int v = 0; v < nin; v++ )
			pos[v].x = ((pos[v].x * 256) / -pos[v].z) + 256 / 2,
			pos[v].y = ((pos[v].y * 256) / pos[v].z) + 212 / 2;
		// draw
		for( int j = 0; j < nin; j++ )
		{
			...
			if ((y0 == y1) || (y0 >= 212) || (y1 < 1)) continue;
			const int iy0 = max( 1, (int)y0 + 1 ), iy1 = min( 212 - 2, (int)y1 );
			...
			{
				if (x0 < xleft[y]) xleft[y] = x0, zleft[y] = z0;
				if (x0 > xright[y]) xright[y] = x0, zright[y] = z0;
				x0 += dx, z0 += dz;
			}
			miny = min( miny, iy0 ), maxy = max( maxy, iy1 );
		}
		for( int y = miny; y <= maxy; xleft[y] = 256 - 1, xright[y++] = 0 )
		{
			float x0 = xleft[y], x1 = xright[y], rxdiff = 1.0f / (x1 - x0);
			float z0 = zleft[y], dz = (zright[y] - z0) * rxdiff;
			const int ix0 = (int)x0 + 1, ix1 = min( 256 - 2, (int)x1 );
			const float f = (float)ix0 - x0;
			z0 += f * dz;
			Pixel* dest = screen->GetBuffer() + y * screen->GetWidth();
			float* zbuf = zbuffer + y * 256;
			for( int x = ix0; x <= ix1; x++, z0 += dz ) // plot span
			{
				if (z0 >= zbuf[x]) continue;
				dest[x] = material->diffuse;
				zbuf[x] = z0;
			}
		}
	}
...
{
	for( size_t s = matList.size(), i = 0; i < s; i++ ) if (matList[i]->name) 
...
{
	for( size_t s = texList.size(), i = 0; i < s; i++ ) if (texList[i]->name)
...
void Scene::LoadMTL( const char* file )
{
	...
	while (!feof( f ))
	{
		...
		else if (!_stricmp( cmd, "kd" ))
		{
			float r, g, b;
			sscanf( line + 3, "%f %f %f", &r, &g, &b );
			current->diffuse = (((int)(r * 255)) << 16) + (((int)(g * 255)) << 8) + (int)(b * 255);
		}
...
SGNode* Scene::LoadOBJ( const char* file, const float _Scale )
{
	struct UniqueVertex 
	{ 
		UniqueVertex( int v, int n = -1 ) : vertex( v ), normal( n ), next( -1 ), subid( 0 ) {}
		int vertex, normal, next, subid, idx;
	};
	...
	if (f) // obj file loader: converts indexed obj file into indexed multi-mesh
	{
		...
		while (!feof( f ))
		{
			...
			while (!feof( f ))
			{
				...
				for ( int i = 0; i < 3; i++ )
				{
					int vidx = v[i] - 1, idx = vidx, lastIdx = idx, nidx = n[i] - 1;
					do
					{
						UniqueVertex& u = unique[idx];
						if (u.subid != subID) // vertex not used before by this mesh
						{
							u.subid = subID, u.next = -1, u.vertex = vidx, u.normal = nidx;
							index.push_back( idx );
							break;
						}
						else if (u.normal == nidx) // vertex used before, but the same
						{
							index.push_back( idx );
							break;
						}
					...
					if (idx != -1) continue;
					uint newIdx = (int)unique.size();
					unique[lastIdx].next = newIdx;
					index.push_back( newIdx );
					unique.push_back( UniqueVertex( vidx, nidx ) );
...
void Rasterizer::Init( Surface* screen )
{
	...
	Mesh::xleft = new float[212], Mesh::xright = new float[212];
	Mesh::uleft = new float[212], Mesh::uright = new float[212];
	Mesh::vleft = new float[212], Mesh::vright = new float[212];
	Mesh::zleft = new float[212], Mesh::zright = new float[212];
	for( int y = 0; y < 212; y++ ) Mesh::xleft[y] = 256 - 1, Mesh::xright[y] = 0;
	zbuffer = new float[256 * 212];
	// calculate view frustum planes
	float C = -1.0f, x1 = 0.5f, x2 = 256 - 1.5f, y1 = 0.5f, y2 = 212 - 1.5f;
	vec3 p0 = { 0, 0, 0 };
	vec3 p1 = { ((x1 - 256 / 2) * C) / 256, ((y1 - 212 / 2) * C) / 256, 1.0f };
	vec3 p2 = { ((x2 - 256 / 2) * C) / 256, ((y1 - 212 / 2) * C) / 256, 1.0f };
	vec3 p3 = { ((x2 - 256 / 2) * C) / 256, ((y2 - 212 / 2) * C) / 256, 1.0f };
	vec3 p4 = { ((x1 - 256 / 2) * C) / 256, ((y2 - 212 / 2) * C) / 256, 1.0f };
...
{
	memset( zbuffer, 0, 256 * 212 * sizeof( float ) );
